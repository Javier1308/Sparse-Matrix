...
public:
    ...
    void insertar(int row, int col, T value) {
        ...
        if (!rows[row]) {
            ...
            if (prev) prev->next_col = nuevoNodo;
            else rows[row] = nuevoNodo;
        }
        ...
        if (!cols[col]) {
            ...
            if (prev) prev->next_row = nuevoNodo;
            else cols[col] = nuevoNodo;
        }
    }
    ...
    void eliminar(int row, int col) {
        ...
        if (current && current->pos_col == col) {
            if (prev) prev->next_col = current->next_col;
            else rows[row] = current->next_col;
        }
        ...
        if (current && current->pos_row == row) {
            if (prev) prev->next_row = current->next_row;
            else cols[col] = current->next_row;
        }
    ...
    bool comparar(SparseMatrix<T>& otra) {
        if (n_rows != otra.n_rows || n_cols != otra.n_cols) return false;
    ...
    SparseMatrix<T> multiplicar(SparseMatrix<T>& otra) {
        if (n_cols != otra.n_rows) throw invalid_argument("Dimensiones incompatibles para multiplicación");
